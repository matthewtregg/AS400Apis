function buildChartGoingDown(startingPgm, entrels) {
  ...
  while (nextLevelChildren.length > 0 && level < 1) {
    newEntLvl = [];
    ...
    for (let cluster of EntLvl) {
      ...
      console.log(EntCluster, "cluster");
  ...
  LeftDiagramData = DiagramGoingDown.map(Lvl => {
...
function getChildId(Obj, LeftDiagramData) {
  let RemEnts = LeftDiagramData.filter(Ent => {
    return Obj.EntityId === Ent.ParentId;
  });
  RemEnts = RemEnts.map(Ent => {
    return Ent.EntityId;
  });
  return RemEnts;
}
...
function buildChartGoingUp(startingPgm, entrels) {
  ...
  console.log(nextLevelParents, "parents");
  let level = 0;
  while (nextLevelParents.length > 0 && level < 2) {
    newEntLvl = [];
    console.log(EntLvl, "EntLvl");
    EntLvl = EntLvl.reduce((acc, Obj) => {
      return acc.concat(Obj);
    });
    for (let cluster of EntLvl) {
      ...
      console.log(nextParents, "Parents");
      EntCluster = [];
...
function addEntClusterToChildren(DiagramGoingDown, DendogramObj) {
  ...
  FirstParentObj = DiagramGoingDown.filter(Obj => {
    return Obj.EntityId === 1;
  })[0];
  DendogramObj.children = FirstParentObj.DiagramChildren.map(Obj => {
...
function addEntClusterToParents(DiagramGoingUp, DendogramObj) {
  ...
  FirstParentObj = DiagramGoingUp.filter(Obj => {
    return Obj.EntityId === 1;
  })[0];
  DendogramObj.children = FirstParentObj.DiagramParents.map(Obj => {
...
function getNestedChildrenGoingDown(Obj, DiagramGoingDown) {
  ...
  let newObj = {};
  newObj.name = Obj;
  nextChildren = DiagramGoingDown.filter(child => {
    return child.DiagramParent === Obj;
  });
...
function getNestedParentsGoingUp(Obj, DiagramGoingUp) {
  ...
  let newObj = {};
  newObj.name = Obj;
  nextChildren = DiagramGoingUp.filter(child => {
    return child.DiagramChild === Obj;
  });
  nextChildren = nextChildren.map(Obj => {
    return Obj.DiagramEntity;
  });
...
function createGoingDownObj(
  ...
  let DiagramChildren = StartingPgmChildRels.map(rel => {
    return rel.CHLD;
  });
